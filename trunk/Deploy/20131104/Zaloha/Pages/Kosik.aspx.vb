...
Partial Class Pages_Kosik
        ...
        Get
            ...
            Else
                If FRV_zakaznik.CurrentMode = FormViewMode.ReadOnly Then
                    _typZakaznika.statID = CType(FRV_zakaznik.FindControl("ddl_stat"), DropDownList).SelectedItem.Value
                    _typZakaznika.statName = CType(FRV_zakaznik.FindControl("ddl_stat"), DropDownList).SelectedItem.Text
                Else
    ...
    Protected Sub RPT_VybraneZbozi_ItemCommand(ByVal source As Object, ByVal e As System.Web.UI.WebControls.RepeaterCommandEventArgs) Handles RPT_VybraneZbozi.ItemCommand
        ...
        If e.CommandName = "Odstranit" Then
            For Each polozka As VybraneZbozi In zbozi
                ...
                End If
            Next
            If zbozi.Count = 0 Then bt_Objednat.Visible = False
        End If
    ...
    Private Sub odeslatObjednavku()
        ...
        If zaznamObjednavky() Then
            ...
            If FRV_zakaznik.CurrentMode = FormViewMode.ReadOnly Then
                sb.Append(String.Format(GetLocalResourceObject("FRV_row1").ToString & " : {0}<br />", RTrim(CType(FRV_zakaznik.FindControl("txb_Prijmeni"), TextBox).Text)))
                sb.AppendLine()
                sb.Append(String.Format(GetLocalResourceObject("FRV_row2").ToString & "    : {0}<br />", RTrim(CType(FRV_zakaznik.FindControl("txb_Jmeno"), TextBox).Text)))
                sb.AppendLine()
                sb.Append(String.Format(GetLocalResourceObject("FRV_row5").ToString & "    : {0}<br />", RTrim(CType(FRV_zakaznik.FindControl("txb_Ulice"), TextBox).Text)))
                sb.AppendLine()
                sb.Append(String.Format(GetLocalResourceObject("FRV_row6").ToString & "    : {0}<br />", RTrim(CType(FRV_zakaznik.FindControl("txb_Mesto"), TextBox).Text)))
                sb.AppendLine()
                sb.Append(String.Format(GetLocalResourceObject("FRV_row7").ToString & "      : {0}<br />", RTrim(CType(FRV_zakaznik.FindControl("txb_PSC"), TextBox).Text)))
                sb.AppendLine()
                sb.Append(String.Format(GetLocalResourceObject("FRV_row8").ToString & "      : {0}<br />", RTrim(typZakaznika.statName)))
                sb.AppendLine()
                sb.Append(String.Format(GetLocalResourceObject("FRV_row4").ToString & "  : {0}<br />", RTrim(CType(FRV_zakaznik.FindControl("txb_Telefon"), TextBox).Text)))
                sb.AppendLine()
                sb.Append("<br /><b>" & GetLocalResourceObject("FRV_title3").ToString & ":</b><br />")
                sb.AppendLine()
                sb.Append(String.Format(GetLocalResourceObject("FRV_row5").ToString & "    : {0}<br />", RTrim(CType(FRV_zakaznik.FindControl("txb_FUlice"), TextBox).Text)))
                sb.AppendLine()
                sb.Append(String.Format(GetLocalResourceObject("FRV_row6").ToString & "    : {0}<br />", RTrim(CType(FRV_zakaznik.FindControl("txb_FMesto"), TextBox).Text)))
                sb.AppendLine()
                sb.Append(String.Format(GetLocalResourceObject("FRV_row7").ToString & "      : {0}<br />", RTrim(CType(FRV_zakaznik.FindControl("txb_FPSC"), TextBox).Text)))
                If Not String.IsNullOrEmpty(RTrim(CType(FRV_zakaznik.FindControl("txb_FUlice"), TextBox).Text)) Then
                    sb.AppendLine()
                    sb.Append(String.Format(GetLocalResourceObject("FRV_row8").ToString & "      : {0}<br />", RTrim(typZakaznika.statName)))
                End If
                sb.AppendLine()
                mailAdr = RTrim(CType(FRV_zakaznik.FindControl("txb_Mail"), TextBox).Text)
            Else
            ...
            Finally
                ...
                CType(Master.FindControl("lbl_cenaNakupu"), Label).Text = Format(0, "### ##0,0 Kč")
            End Try
    ...
    Private Function zaznamObjednavky() As Boolean
        ...
        Dim komu As MailAddress = New MailAddress("petrsmazal@gmail.com")
    ...
    Protected Sub RBL_odber_SelectedIndexChanged(ByVal sender As Object, ByVal e As System.EventArgs) Handles RBL_odber.SelectedIndexChanged
        ...
        If IsNothing(zbozi) Then Exit Sub
        sumaObj = obj.sectiPolozky(zbozi, CInt(RBL_odber.SelectedValue), prihlasen.VC, lang)
        CType(Master.FindControl("lbl_cenaNakupu"), Label).Text = Format(sumaObj, "### ##0,00 Kč")
        RPT_VybraneZbozi.DataSource = zbozi
        RPT_VybraneZbozi.DataBind()
    End Sub
