...
Partial Class Pages_Kosik
        ...
        If zaznamObjednavky() Then
            ...
            If typZakaznika.statName = "Czech Republic" Then
                ...
                If RBL_odber.SelectedValue = "3" Then 'PayPal
                    sb.Append("<tr><td colspan='" + colspan + "'  align='right'>" & GetLocalResourceObject("code_text16").ToString & " &nbsp;-&nbsp;</td><td align='right'>" + Format((postovne() + sumaObj) * 0.03, "#,0 Kč") + "</td></tr>")
                    sb.Append("<tr><td colspan='" + colspan + "'  align='right'>" & GetLocalResourceObject("RPT_VybraneZbozi_Col4").ToString & " &nbsp;-&nbsp;</td><td align='right' style='background-color:#fdeaa5;'>" + Format(postovne() + sumaObj + ((postovne() + sumaObj) * 0.029999999999999999), "#,0 Kč") + "</td></tr>")
                Else
    ...
    Private Sub setCenaCelkem()
        ...
        ElseIf typZakaznika.statName = "Slovakia" Then
            If RBL_odber.SelectedIndex = 0 Then
                CType(lbl_cenaCelkem, Label).Text += GetLocalResourceObject("cenaCelkemDobirkaSK").ToString '" + cena dopravy podle hmotnosti a dobírkové částky min. 280 Kč"
            ElseIf RBL_odber.SelectedIndex = 1 Then
                CType(lbl_cenaCelkem, Label).Text += GetLocalResourceObject("cenaCelkemPrevodSK").ToString '" + cena dopravy podle hmotnosti min. 220 Kč"
            ElseIf RBL_odber.SelectedIndex = 2 Then
                CType(lbl_cenaCelkem, Label).Text += GetLocalResourceObject("cenaCelkemPPSK").ToString '" + cena dopravy podle hmotnosti + poplatek PayPal 3% z ceny zboží a dopravy min. 220 Kč"
            End If
        Else 'cizina
    ...
    Public Sub zpusobDoruceni()
        ...
        If CultureInfo.CurrentCulture.ToString = "cs-CZ" Then
            ...
            cenyDoruceniSK = New String() {"(od 212,- Kč, podle hmotnosti a dobírkové částky)", "<br>(od 170,- Kč, podle hmotnosti)", "(od 170,- Kč, podle hmotnosti)", "(0,- Kč)"}
        ...
        Else
            ...
            zpusobDoruceniSK = New String(,) {{"Cash on delivery GLS ",
                                                    ...
                                                    "PayPal, GLS ",
                                                    "Cash, personal collection "}, {"0", "1", "3", "4"}}
            cenyDoruceniSK = New String() {"(at least 280,- Kč, by freight cost and weight )", "<br>(at least 220,- Kč, by weight)", "(at least 220,- Kč, by weight)", "(0,- Kč)"}
            zpusobDoruceniEN = New String(,) {{"Bank transfer",
                                                                "PayPal"}, {"1", "3"}}
        End If
