...
Partial Class MasterPage_MasterPage
    ...
    Private Sub SetNextImage()
        ...
        imgSlideShow.ImageUrl = imgUrlString
        SlideShowLink.NavigateUrl = navigateURLString
        imgSlideShow.AlternateText = alternateTextString
    End Sub

    Private Sub getImgFromXML()
        Dim doc As XmlDocument = New XmlDocument
        Dim path As String
        Dim imgs As XmlNodeList
        Dim imgPosice As Integer = 0
        
        imgPosice = CType(Session("posledniObrazek"), Integer)
        If imgPosice >= 6 Then
            imgPosice = 0
        Else
        ...
        Session("posledniObrazek") = imgPosice

        Try

            path = Server.MapPath("~/SlideShow/Advertisements.xml")
            doc.Load(path)
        Catch ex As FileNotFoundException

        End Try

        imgs = doc.DocumentElement.GetElementsByTagName("Ad")
        imgUrlString = imgs(imgPosice).ChildNodes.Item(0).InnerText
        navigateURLString = imgs(imgPosice).ChildNodes.Item(1).InnerText
        alternateTextString = imgs(imgPosice).ChildNodes.Item(2).InnerText
