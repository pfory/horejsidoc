...
<asp:Content ID="Content1" ContentPlaceHolderID="Hlavni" Runat="Server">
    ...
    ProviderName="<%$ ConnectionStrings:DataHorejsi.ProviderName %>" 
        SelectCommand="SELECT DISTINCT ID_zakaznik, LTRIM(RTRIM(Prijmeni)) + ' ' + LTRIM(RTRIM(Jmeno)) AS Zakaznik, Mail, VC, t_kontakty.Obchodnik, Firma, IC, DIC,Uzivatel,Heslo,Jazyk, T_Kontakty.DatumRegistrace FROM T_Zakaznik left outer join T_Kontakty on T_Zakaznik.Mail=T_Kontakty.MailAdresa WHERE (1 = 1) AND (Registrovan = 1)" 
    ...
    <asp:GridView ID="GV_Zakaznici" runat="server" AllowPaging="True" AutoGenerateColumns="False"
        ...
        <Columns>
            ...
            <asp:BoundField DataField="Heslo" HeaderText="Heslo"  
                ReadOnly="True" />        
            <asp:BoundField DataField="DatumRegistrace" DataFormatString="{0:d}" HeaderText="Datum registrace" ReadOnly="True" />
