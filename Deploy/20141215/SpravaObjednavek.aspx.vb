...
Partial Class Pages_SpravaObjednavek
    ...
    Private Sub odesliPotvrzeni(ByVal ID_objednavky As Integer)
        ...
        PHE = 0.0
            ...
            ElseIf infoObj.statID = 199 And infoObj.zpusobDoruceni <> 4 Then 'Slovensko
                ...
                If infoObj.zpusobDoruceni = 0 Or infoObj.zpusobDoruceni = 1 Or infoObj.zpusobDoruceni = 3 Then
                  If Trim(infoObj.idBaliku) = String.Empty Then
                    bodyMail.Append(String.Format("Zásilka byla odeslána službou GLS.", Trim(infoObj.idBaliku)))
                  Else
                    bodyMail.Append(String.Format("Zásilka číslo <b>{0}</b> byla odeslána službou GLS.", Trim(infoObj.idBaliku)))
                  End If
                Else
                  If Trim(infoObj.idBaliku) = String.Empty Then
                    bodyMail.Append(String.Format("Zásilka byla odeslána službou PPL.", Trim(infoObj.idBaliku)))
                  Else
                    bodyMail.Append(String.Format("Zásilka číslo <b>{0}</b> byla odeslána službou PPL.", Trim(infoObj.idBaliku)))
                 End If
               End If
            Else
            ...
            If infoObj.zpusobDoruceni <= 3 And infoObj.statID = 58 And soucet <> 0.0 Then
                ...
                bodyMail.AppendLine()
            End If
            If (infoObj.zpusobDoruceni = 0 Or infoObj.zpusobDoruceni = 1 Or infoObj.zpusobDoruceni = 3) And infoObj.statID = 199 And soucet <> 0.0 Then
                bodyMail.Append("<br/>")
                bodyMail.Append("Na Váš email obdržíte zprávu od společnosti GLS CZ s odkazem na sledování zásilky.")
                'bodyMail.Append(String.Format("Balík č.<b>{0}</b> můžete sledovat na webu České pošty - sledování zásilek <br/><br/>", Trim(infoObj.idBaliku)))
                'bodyMail.Append("Sledování zásilek : <a href='http://www.cpost.cz/cz/nastroje/sledovani-zasilky.php'>Odkaz na sledování zásilek</a> (Zásilka se objeví v evidenci nejdříve dnes večer).")
                bodyMail.AppendLine()
            End If
            ...
            'odeslano = email.OdesliMail(od:=New MailAddress("objednavky@horejsi.cz", "e-Obchod Hořejší"), komu:=New MailAddress(infoObj.mail), predmet:=HttpContext.GetGlobalResourceObject("MailText", "mailPredmet", cult).ToString, zprava:=bodyMail.ToString, kopie:=New MailAddress("mr.datel@gmail.com"))
